...
# OpenAI-compatible LLM API settings (These will now be configurable via arguments, defaults are kept)
# Defaults to LM Studio API
...
DEFAULT_MODEL_NAME = "orpheus-3b-0.1-ft"
MAX_TOKENS = 1200
TEMPERATURE = 0.6
TOP_P = 0.9
...
def main():
    ...
    parser.add_argument("--api-url-prefix", type=str, default=DEFAULT_API_URL_PREFIX,
                        help=f"API URL prefix (e.g., http://your-lm-studio-host:port), default: {DEFAULT_API_URL_PREFIX}")
    ...
    app.run(host=args.host, port=args.port, debug=args.debug, threaded=False,
            processes=1)  # important threaded=False and processes=1 for LM Studio
